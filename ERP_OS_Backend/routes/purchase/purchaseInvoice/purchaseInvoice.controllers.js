...
const getAllPurchaseInvoice = async (req, res) => {
  if (req.query.query === "info") {
    ...
    try {
      ...
      const allPurchaseInvoice = await prisma.purchaseInvoice.findMany({
        include: {
          supplier: true,
        },
      });
    ...
    try {
      // get all purchase invoice
      const allPurchaseInvoice = await prisma.purchaseInvoice.groupBy({
        orderBy: {
          date: "asc",
        },
        by: ["date"],
...
const updateSinglePurchaseInvoice = async (req, res) => {
  try {
    res.json({ message: "update purchase invoice not implemented" });
  } catch (error) {
    res.status(400).json(error.message);
    console.log(error.message);
  }
...
const deleteSinglePurchaseInvoice = async (req, res) => {
  try {
    ...
    if (purchaseInvoice.status === true) {
      // product quantity decrease
      purchaseInvoice.purchaseInvoiceProduct.forEach(async (item) => {
        await prisma.product.update({
          where: {
            id: Number(item.product_id),
          },
      ...
      const [deletePurchaseInvoice, transaction] = await prisma.$transaction([
        ...
        prisma.transaction.create({
          data: {
            ...
            debit_id: 1,
            credit_id: 3,
            amount: parseFloat(purchaseInvoice.paid_amount),
            particulars: `paid amount get refunded of purchase invoice no. ${req.params.id}`,
      ...
      res.json({ message: "purchase invoice already deleted" });
    }
  } catch (error) {
    res.status(400).json(error.message);
    console.log(error.message);
